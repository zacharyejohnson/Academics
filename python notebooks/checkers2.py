...
class Checkers(): 
    ...
    def play(self): 
        ...
        while len(agents_dict[turn]) > 0:
            ...
            if not skip: 
                ...
                avaliable_agents = list (moves_dict.keys())
                if len(avaliable_agents) > 0:
                    rand_agent_ind = np.random.randint(0, len(avaliable_agents))
                    agent = avaliable_agents[rand_agent_ind]
    ...
    def minimax(self, depth, max_player, game):
        if depth == 0 or len(self.agents_dict['red']) == 0 or len(self.agents_dict['black']) == 0 :
